...
/**
 * Reduces a net system by removing places and transitions that are given as a projection set.
 ...
 *
 * Based on the org.jbpt.petri.structure.PetriNetProjector class - rules a) and c) mark the
 * succeeding places if the preceeding place will be remove, rule b) removes the succeeding
 * places if they are not marked
 * 
 ...
 */
public class NetSystemProjector<F extends IFlow<N>, N extends INode, P extends IPlace, T extends ITransition, M extends IMarking<F, N, P, T>> {

	public void reducePetriNetBasedOnProjectionSet(INetSystem<F, N, P, T, M> pn, Set<T> projectionSet) {
		...
		for (P p1 : pn.getPlaces()) {
			...
			for (P p2 : pn.getPlaces()) {
				...
				if (preset.size() == 1 && postset.size() == 1 && preset2.size() == 1 && postset2.size() == 1) {
	...
	private void applyReductionRuleA(INetSystem<F, N, P, T, M> pn, T t) {
		...
		// if the place that will be removed is marked, mark the succeeding place
		if (pn.isMarked(pre))
			pn.getMarking().put(suc, pn.getMarking().get(pre));

		pn.removePlace(pre);
		pn.removeTransition(t);
	}

	private void applyReductionRuleB(INetSystem<F, N, P, T, M> pn, T t) {
		Set<P> prePlaces = pn.getPreset(t);
		P sucPlace = pn.getPostset(t).iterator().next();
		T sucT = (T) pn.getPostset(sucPlace).iterator().next();

		// remove the succeeding place if it is not marked
		if (!pn.isMarked(sucPlace)) {
	...
	private void applyReductionRuleC(INetSystem<F, N, P, T, M> pn, T t) {
		...
		// if the place that will be removed is marked, mark the succeeding places
		if (pn.isMarked(prePlace)) {
			for (P suc : sucPlaces)
				pn.getMarking().put(suc, pn.getMarking().get(prePlace));
		}
